...
async function handleRequest(
  url: string,
  method: string,
  body?: LoginData | RegisterData | { email?: string; token?: string; newPassword?: string }
...
export function useUser() {
  ...
  const requestPasswordResetMutation = useMutation<RequestResult, Error, { email: string }>({
    mutationFn: (data) => handleRequest('/api/request-password-reset', 'POST', data),
  });
